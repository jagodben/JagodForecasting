...
public static class MatchupBlender
{
    ...
    /// matchup qualifies; name comparison is delegated to <see cref="CandidateNames"/>, which
    /// tolerates the suffix and short-form variants Wikipedia's headers use freely.
    /// </summary>
    private static bool MatchesNominee(string? candidate, string? nominee)
    {
        if (nominee is null) return true;
        return CandidateNames.Match(candidate, nominee);
    }
}
